...
export default function ClientHome({ client, setCurrentView }) {
  ...
  const loadProgressData = async () => {
    ...
    // Use the correct client ID field
    const clientId = client.id || client.legacy_id
    if (!clientId) {
      console.error('No valid client ID found')
      return
    }
    
    try {
      // Load client's workout plan
      ...
      // Load today's calories from meal tracking
  ...
  const loadAccountabilityData = async () => {
    ...
    try {
      ...
      // Load coach notifications from database
