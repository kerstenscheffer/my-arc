  ...
  useEffect(() => {
    async function loadTemplates() {
      try {
        ...
        console.log(`📚 Loaded ${data?.length || 0} existing templates`)
      } catch (e) {
...
useEffect(() => {
  ...
  ;(async () => {
    try {
      ...
      // 3. Get template structure if needed
...
const handleSaveCurrentPlan = async () => {
  ...
  try {
    ...
    // 2. Check of er al overrides bestaan
    const { data: existing } = await supabase
      .from('client_meal_overrides')
      .select('id')
      .eq('client_id', selectedClient.id)
      .eq('plan_id', clientPlan.id)
      .single()
    ...
    alert('❌ Plan opslaan mislukt: ' + e.message)
  } finally {
    setSaving(false)
  }
}
