...
function AccountabilityAlert({ alert, onDismiss, onAction }) {
  const getAlertStyle = () => {
    switch(alert.type) {
      case 'warning': return { bg: '#7f1d1d', border: '#ef4444', icon: '⚠️' }
      case 'reminder': return { bg: '#78350f', border: '#f59e0b', icon: '🔔' }
      case 'coach': return { bg: '#064e3b', border: '#10b981', icon: '💬' }
      case 'achievement': return { bg: '#78350f', border: '#fbbf24', icon: '🏆' }
      default: return { bg: '#1a1a1a', border: '#10b98133', icon: '📢' }
    }
  }
  ...
  return (
    <div style={{
      ...
      gap: '1rem',
      padding: '1rem',
      background: `linear-gradient(135deg, ${style.bg} 0%, rgba(0,0,0,0.3) 100%)`,
      borderRadius: '8px',
...
export default function ClientHome({ client, setCurrentView }) {
  ...
  const loadFeaturedVideo = async () => {
    ...
    try {
      ...
      console.log('No featured video available')
    }
  }
  ...
  const loadProgressData = async () => {
    ...
    if (!clientId) {
      ...
      // Don't return - still try to load what we can
    }
    
    console.log('Using client ID for queries:', clientId)
    
    try {
      // Load client's workout plan - only if we have a valid UUID
      if (clientId) {
        ...
        if (!planError && clientPlan && clientPlan.workout_schemas) {
          ...
          if (todayWorkout) {
            // Get the first muscle group for display
      ...
      // Load goals - only if we have a valid UUID
      if (clientId) {
      ...
      if (goals && goals.length > 0) {
        ...
        if (calorieGoalData) {
          setCalorieGoal(calorieGoalData.target_value || 2000)
        }
      } else {
        // Set defaults from client data if no goals exist
      ...
      // Load this week's workouts - only if we have a valid UUID
      if (clientId) {
      ...
      // Load today's calories from meal tracking - only if we have a valid UUID
      if (clientId) {
        const todayStr = new Date().toISOString().split('T')[0]
        const { data: mealProgress } = await supabase
          .from('meal_progress')
          .select('*')
          .eq('client_id', clientId)
          .eq('date', todayStr)
        
        if (mealProgress && mealProgress.length > 0) {
          const totalCalories = mealProgress.reduce((sum, meal) => 
              ...
              <div style={{
                ...
                <div style={{ 
                  ...
                  <span style={{ color: '#f59e0b', fontWeight: 'bold' }}>
                    {calorieGoal > todayCalories ? 'Calorieën Over' : 'Calorie Status'}
                  </span>
                </div>
                <p style={{ color: '#fff', fontSize: '1.1rem' }}>
                  {calorieGoal > todayCalories 
                    ? `${Math.max(0, calorieGoal - todayCalories)} kcal` 
                    : `${Math.abs(todayCalories - calorieGoal)} kcal over`}
                </p>
                <button
                  onClick={() => setCurrentView('mealplan')}
                  style={{
