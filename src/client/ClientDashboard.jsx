...
export default function ClientDashboard() {
 ...
 { 
    id: 'calls', 
    label: t('coaching_calls', 'Coaching Calls'), 
    iconUrl: '/icons/call-icon.svg' // Of gebruik een emoji: '🗓️'
  } 
 ]
  ...
  const loadClientData = async () => {
    try { 
      ...
      // ⭐ Sla client data ook op in localStorage als backup
  ...
  // ⭐ BEHOUD DEZE LOADING/ERROR STATES - Ze zijn mooi gestyled!
  if (loading) {
    return (
...
<div className="myarc-animate-in">
  ...
  {currentView === 'mealplan' && (
    <ClientMealPlan
      ...
      db={DatabaseService}  // ⭐ VOEG DIT TOE!
    />
  )}
  ...
  {currentView === 'workout' && (
    <ClientWorkoutPlan
      client={client}
      schema={schema}
      db={DatabaseService}  // ⭐ Ook hier voor consistency
    />
  )}
...
{currentView === 'progress' && (
  ...
  {currentView === 'calls' && (
    <ClientCalls
      db={DatabaseService}  // ✅ Deze heeft het al goed!
      clientInfo={client}
    />
  )}
            {currentView === 'profile' && (
