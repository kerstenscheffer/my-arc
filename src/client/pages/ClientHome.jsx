...
// TRAJECT PROGRESS
// ============================================
function TrajectProgress({ client }) {
  const isMobile = useIsMobile()
  const startStr = client?.coaching_start_date
  const endStr = client?.subscription_end_date
  ...
  // Dagen tussen start en eind voor totaal-trackmeter.
  let percentage = null
  let weeksIn = null
  let weeksTotal = null
  let daysLeft = null
  ...
  if (start && end) {
    ...
    percentage = Math.min(100, Math.round((elapsed / total) * 100))
    weeksIn = Math.max(0, Math.floor(elapsed / 7))
    weeksTotal = Math.ceil(total / 7)
    daysLeft = Math.max(0, Math.ceil((end.getTime() - today.getTime()) / 86400000))
  } else if (start) {
    // Open-ended traject: alleen start-datum bekend. Toon hoe lang client al loopt.
    const elapsed = Math.max(0, (today.getTime() - start.getTime()) / 86400000)
    weeksIn = Math.max(0, Math.floor(elapsed / 7))
    daysIn = Math.max(0, Math.floor(elapsed))
  } else if (end) {
    daysLeft = Math.max(0, Math.ceil((end.getTime() - today.getTime()) / 86400000))
  }

  const endLabel = end?.toLocaleDateString('nl-NL', { day: 'numeric', month: 'short', year: 'numeric' })
  const startLabel = start?.toLocaleDateString('nl-NL', { day: 'numeric', month: 'short', year: 'numeric' })

  return (
    <div>
      <SectionLabel icon={Clock} label="Jouw traject" isMobile={isMobile} />
      <div style={{ padding: isMobile ? '0 1rem' : '0 1.5rem' }}>
      <div style={{
        background: 'rgba(255,255,255,0.03)',
        border: '1px solid rgba(255,255,255,0.08)',
        ...
        <div style={{
          ...
          marginBottom: percentage != null ? '0.75rem' : 0,
        }}>
          {weeksIn != null && weeksTotal != null && (
            <div>
              <div style={{
                ...
                {weeksIn}
                <span style={{ color: 'rgba(255,255,255,0.3)', fontWeight: 700 }}>/{weeksTotal}</span>
              </div>
              <div style={{
          ...
          {weeksIn != null && weeksTotal == null && (
            <div>
              ...
              <div style={{
                ...
                {weeksIn === 1 ? 'Week bezig' : 'Weken bezig'}
              </div>
            </div>
          )}
          {daysLeft != null && (
