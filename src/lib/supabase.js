...
export async function getClientSchema(clientId) {
  try {
    console.log('📋 Getting schema for client:', clientId)
...
export async function getClientByEmail(email) {
  try {
    ...
    if (!clients || clients.length === 0) {
      console.log('⚠️ No clients found with exact match')
      ...
      console.log('📊 Case insensitive results:', clientsInsensitive)
      
      if (clientsInsensitive && clientsInsensitive.length > 0) {
        console.log('✅ Found client with case-insensitive match')
      ...
      console.log('⚠️ No client found for:', email)
      
      return null
    }
    
    console.log(`✅ Found ${clients.length} client(s), returning first`)
