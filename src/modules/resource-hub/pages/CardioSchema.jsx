// src/modules/resource-hub/pages/CardioSchema.jsx
...
export default function CardioSchema({ db, bundel, user, client, isMember, navigate }) {
  ...
  if (!isMember) return (
    <div style={{ minHeight: '100vh', background: '#000', color: '#fff', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '2rem', textAlign: 'center' }}>
      <Lock size={32} color={G.primary} style={{ marginBottom: '1rem', opacity: 0.6 }} />
      <h2 style={{ margin: '0 0 0.5rem', fontSize: '1.25rem', fontWeight: '800' }}>8-Weken Opbouwschema</h2>
      <p style={{ margin: '0 0 1.5rem', fontSize: '0.85rem', color: 'rgba(255,255,255,0.35)', maxWidth: '300px', lineHeight: 1.5 }}>Exclusief voor MY ARC leden. Word lid en krijg toegang tot alle schema's.</p>
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#000', color: '#fff', padding: isMobile ? '1rem' : '1.5rem' }}>
      ...
      <h1 style={{ margin: '0 0 0.25rem', fontSize: isMobile ? '1.4rem' : '1.6rem', fontWeight: '900' }}>8-Weken Opbouwschema</h1>
      <p style={{ color: 'rgba(255,255,255,0.35)', fontSize: '0.85rem', marginBottom: '2rem' }}>Selecteer een cardiovorm en bekijk je schema.</p>
      <div style={{ padding: '3rem', textAlign: 'center', borderBottom: `1px solid ${G.border}`, borderTop: `1px solid ${G.border}` }}>
        <p style={{ color: 'rgba(255,255,255,0.2)', fontSize: '0.85rem' }}>Schema content wordt binnenkort toegevoegd</p>
      </div>
    </div>
  )
}
