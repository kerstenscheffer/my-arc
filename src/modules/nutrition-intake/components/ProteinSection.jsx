// FILE: src/modules/nutrition-intake/components/ProteinSection.jsx
// ProteinSection.jsx - LOCKED DEFAULT + unlock option
import React, { useState, useEffect } from 'react'
import { intakeTheme, responsive, getCardStyle } from '../styles/intake-theme'

const STANDARD_PROTEIN_SOURCES = [
  { id: 'kip', name: 'Kipfilet', reason: 'Mager, veelzijdig, hoge biologische waarde' },
  { id: 'eieren', name: 'Eieren', reason: 'Compleet eiwit, rijk aan essentiële aminozuren' },
  { id: 'kwark', name: 'Magere kwark', reason: 'Langzaam verteerbaar, perfect voor bed' },
  { id: 'zalm', name: 'Zalm', reason: 'Omega-3 bonus, hoogwaardig eiwit' },
  { id: 'rund', name: 'Magere rundvlees', reason: 'IJzer, zink, creatine natuurlijk aanwezig' },
  { id: 'whey', name: 'Whey protein', reason: 'Snelle opname, convenience' }
]

export default function ProteinSection({ value, onChange, onComplete, isMobile }) {
  ...
  const handleAcceptStandard = () => {
    const preferences = {
      standard_list: STANDARD_PROTEIN_SOURCES.map(s => s.name),
      removals: [],
      additions: [],
      notes: null
    }
  ...
  const handleWantToCustomize = () => {
    setWantsToCustomize(true)
  }
  
  const handleToggleRemoval = (sourceId) => {
    const updated = removals.includes(sourceId) 
      ? removals.filter(id => id !== sourceId)
      : [...removals, sourceId]
    setRemovals(updated)
  ...
  return (
    <div style={getSectionContainerStyle(isMobile)}>
      <div style={getSectionHeaderStyle(isMobile)}>
        <h2 style={getSectionTitleStyle(isMobile)}>2. Eiwitbronnen</h2>
        <p style={getSectionDescriptionStyle(isMobile)}>
          Deze eiwitbronnen zijn gekozen voor optimale spiergroei en herstel.
        </p>
      </div>
      ...
      {!wantsToCustomize ? (
        <div style={getLockedCardStyle(isMobile)}>
          <div style={getLockedHeaderStyle(isMobile)}>
            ...
            <div style={getLockedTitleStyle(isMobile)}>STANDAARD EIWITBRONNEN</div>
          </div>
          
          <div style={getStandardListStyle(isMobile)}>
            {STANDARD_PROTEIN_SOURCES.map(source => (
          ...
          <div style={getInfoBoxStyle(isMobile)}>
            💡 Deze bronnen zorgen voor complete eiwit inname verspreid over de dag
          </div>
          
          <div style={getButtonGroupStyle(isMobile)}>
            <button onClick={handleAcceptStandard} style={getPrimaryButtonStyle(isMobile)}>
              ✓ Dit is prima
            </button>
            <button onClick={handleWantToCustomize} style={getSecondaryButtonStyle(isMobile)}>
              Aanpassen
            </button>
          </div>
        </div>
        ...
        <>
            ...
            <div style={getRemovalListStyle(isMobile)}>
              {STANDARD_PROTEIN_SOURCES.map(source => (
                <label key={source.id} style={getCheckboxLabelStyle(isMobile)}>
                  <input 
                    type="checkbox" 
                    checked={removals.includes(source.id)}
                    onChange={() => handleToggleRemoval(source.id)}
          ...
          <div style={getCustomCardStyle(isMobile)}>
            ...
            <textarea
              ...
              placeholder="Bijv: Tonijn uit blik, Kalkoenfilet, Tofu"
          ...
          <div style={getCustomCardStyle(isMobile)}>
            ...
            <textarea
              ...
              placeholder="Bijv: Allergie voor schaaldieren, liever geen vis..."
              style={getTextareaStyle(isMobile)}
              rows={3}
            />
          </div>
