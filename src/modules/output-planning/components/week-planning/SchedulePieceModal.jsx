// src/modules/output-planning/components/week-planning/SchedulePieceModal.jsx
// Modal to schedule a content piece into a week with phase timing
// FIXED: TIME_OPTIONS are objects with {value, label}, safe arrays

import { useState } from 'react'
import { X, FileText, Video, Scissors, Send } from 'lucide-react'
...
export default function SchedulePieceModal({
  isOpen,
  onClose,
  onSave,
  piece,
  ...
  // Initialize schedule for each required phase
  const [phaseSchedule, setPhaseSchedule] = useState(() => {
    const initial = {}
    requiredPhases.forEach((phase, idx) => {
      const dayIndex = Math.min(idx, (safeWeekDays.length || 1) - 1)
      initial[phase] = {
        dayOfWeek: safeWeekDays[dayIndex]?.dayOfWeek || 'monday',
        time: idx === 0 ? '09:00' : idx === 1 ? '10:00' : idx === 2 ? '14:00' : '16:00',
        duration: phase === 'shoot' ? 60 : 30
      }
    })
    return initial
  })
        ...
        <div style={{
          ...
          {requiredPhases.length === 0 ? (
            <p style={{ color: 'rgba(255, 255, 255, 0.5)', textAlign: 'center' }}>
              Deze content piece heeft geen fases nodig.
            </p>
          ) : (
