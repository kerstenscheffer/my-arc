...
// Welke kant gaat het op, en met welke marges? Afgeleid uit wat er van de klant
// bekend is; een expliciete keuze per klant kan later bovenop deze afleiding.
export function maakConfig(client, overrides = {}) {
  ...
  if (start && doel) {
    ...
    if (Math.abs(verschil) < start * 0.01) richting = 'recomp'
    else if (verschil > 0) richting = 'aankomen'
  }

  // Aankomen hoort langzamer te gaan dan afvallen: sneller is vooral vet.
  ...
  // Leaner of ouder: eerder ingrijpen bij te snel, want spierverlies ligt op de
  // loer. Leeftijd staat niet altijd in het dossier; dan telt alleen vetpercentage.
  ...
  return { ...STANDAARD, ...basis, richting, kwetsbaar, ...overrides }
}
...
// De drie lijnen op week n. `traag` en `snel` zijn de grenzen; welke van de
// twee de hoogste waarde heeft hangt af van de richting.
export function lijnenOpWeek(n, startGewicht, config) {
  ...
  const perWeek = (pct) => startGewicht * (pct / 100) * n * teken
  if (config.richting === 'recomp') {
  ...
  return {
    doel: startGewicht + perWeek(config.streeftempo_pct),
    traag: startGewicht + perWeek(config.traagste_pct),
    snel: startGewicht + perWeek(config.snelste_pct),
  }
}
