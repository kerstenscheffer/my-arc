...
import {
  ...
  Send, FileText, Percent, UserX, Eye, Download, LineChart as LineChartIcon, Hourglass,
...
export default function WeekStatsModal({ isOpen, onClose, leadService, coachId, isMobile: propMobile, onTargetsSaved }) {
  ...
  const countItems = [
    ...
    { label: 'Toegezegd', value: '€' + Math.round(funnel?.sale?.toegezegd || 0).toLocaleString('nl-NL'), Icon: Hourglass, color: '#FFD700', info: 'Deals die je deze periode sloot maar waarvan het geld nog niet binnen is: de ja staat er, de betaling niet. Voer je de betaling in (calls-venster, tabblad Betaling), dan schuift dit bedrag naar Orderwaarde en Binnengekomen.' },
    { label: 'Binnengekomen', value: cash ? '€' + Math.round(cash.bedrag).toLocaleString('nl-NL') : '—', Icon: Wallet, color: '#22c55e', info: 'Wat er deze periode aan geld binnenkwam: aanbetalingen, restbetalingen en maandtermijnen waarvan de betaaldatum is verstreken. Let op — dit is het betaalschema, niet een bevestigde incasso. Er is geen koppeling met de bank, dus het zegt dat de afgesproken datum geweest is. Sales die nog op betaling wachten tellen niet mee.' + (cash?.verwacht ? ` Er staat nog €${Math.round(cash.verwacht).toLocaleString('nl-NL')} open in deze periode.` : '') },
              ...
              {revLoading ? (
                ...
                return (
                  <>
                    ...
                    {/* Een echte grafiek: y-as met bedragen en rasterlijnen,
                        x-as met de maanden, en dikke staven die elkaar bijna
                        raken. Wit is geld dat binnen is — dat is de hoofdrol;
                        goud en stippels zijn de uitzonderingen. */}
                    <div style={{ display: 'flex', gap: isMobile ? 8 : 12 }}>
                      {/* Y-as */}
                      <div style={{ width: isMobile ? 44 : 56, flexShrink: 0, height: BALK_H, position: 'relative' }}>
                        {ticks.map(t => (
                          <div key={t} style={{
                            position: 'absolute', right: 0, bottom: Math.round((t / asMax) * BALK_H),
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ position: 'relative', height: BALK_H }}>
                          ...
                          {/* De staven: dik en tegen elkaar aan. */}
                          <div style={{
                            position: 'absolute', inset: 0, zIndex: 1,
                            display: 'flex', gap: 3, alignItems: 'flex-end',
                          }}>
                            {zichtbaar.map(m => {
                              const hoogte = (bedrag) => Math.round((bedrag / asMax) * BALK_H)
                              const h = m.amount > 0 ? Math.max(2, hoogte(m.amount)) : 0
                              ...
                              return (
                                <div
                                  key={m.key}
                                  title={`${m.label}: ${eur(m.amount)}${m.pendingAmount > 0 ? ` · ${eur(m.pendingAmount)} toegezegd` : ''}`}
                                  style={{ flex: 1, minWidth: 0, height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}
                                >
                                  {openDeel > 0 && (
                                    <div style={{
                                      width: '100%', height: openDeel,
                                      border: '1px dashed rgba(255,255,255,0.45)',
                                      borderBottom: h > 0 ? 'none' : '1px dashed rgba(255,255,255,0.45)',
                    ...
                    {/* Legenda: alleen wat in beeld staat. */}
...
function kerncijfers({ activity, funnel, reactionStats, cash } = {}) {
  ...
  return [
    ...
    { key: 'toegezegd',    label: 'Toegezegd',     waarde: Math.round(n(funnel?.sale?.toegezegd)), soort: 'euro' },
