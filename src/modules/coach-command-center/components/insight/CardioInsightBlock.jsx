    ...
    <div style={{ borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
      <button onClick={() => setOpen(o => !o)} style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: m ? '0.5rem 0.75rem' : '0.625rem 1rem', background: 'transparent', border: 'none', cursor: 'pointer', touchAction: 'manipulation', WebkitTapHighlightColor: 'transparent' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
          <Footprints size={13} color={ORANGE} />
          <span style={{ fontSize: '0.7rem', fontWeight: 700, color: ORANGE, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Cardio</span>
          <span style={{ fontSize: '0.55rem', fontWeight: 700, color: 'rgba(255,215,0,0.6)' }}>{thisWeek} deze week · {logs.length} in 30d</span>
        </div>
      ...
      {open && (
        <div style={{ padding: m ? '0 0.75rem 0.5rem' : '0 1rem 0.625rem', display: 'flex', flexDirection: 'column', gap: '0.3rem' }}>
          {logs.slice(0, 12).map(log => (
            <div key={log.id} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.35rem 0.5rem', background: 'rgba(255,215,0,0.05)', border: '1px solid rgba(255,215,0,0.2)', borderRadius: '6px' }}>
              <span style={{ fontSize: '0.55rem', fontWeight: 700, color: 'rgba(255,255,255,0.35)', minWidth: m ? '38px' : '42px' }}>{fmtDate(log.logged_date)}</span>
              ...
              <span style={{ fontSize: '0.6rem', fontWeight: 700, color: ORANGE, flexShrink: 0, whiteSpace: 'nowrap' }}>
                {[
          ...
          {logs.length > 12 && (
            <div style={{ fontSize: '0.55rem', color: 'rgba(255,255,255,0.25)', textAlign: 'center', paddingTop: '0.2rem' }}>+{logs.length - 12} eerder</div>
          )}
        </div>
      )}
    </div>
