  ...
  async getRecentActivities(clientIds, hoursBack = 24) {
    ...
    try {
      ...
      const [
        ...
        this.supabase.from('ai_meal_progress').select('*').in('client_id', clientIds).gte('updated_at', since).order('updated_at', { ascending: false }).limit(20),
        this.supabase.from('workout_sessions').select('*').in('client_id', clientIds).gte('completed_at', since).not('completed_at', 'is', null).order('completed_at', { ascending: false }).limit(20),
        this.supabase.from('weight_history').select('*').in('client_id', clientIds).gte('created_at', since).order('created_at', { ascending: false }).limit(20),
        this.supabase.from('water_tracking').select('*').in('client_id', clientIds).gte('updated_at', since).order('updated_at', { ascending: false }).limit(20),
        this.supabase.from('ai_mood_logs').select('*').in('client_id', clientIds).gte('created_at', since).order('created_at', { ascending: false }).limit(20),
        this.supabase.from('progress_photos').select('*').in('client_id', clientIds).gte('created_at', since).eq('is_private', false).order('created_at', { ascending: false }).limit(20),
      ])
      ...
      weightData?.forEach(weight => activities.push({
        id: `weight_${weight.id}`, clientId: weight.client_id, type: 'weight', icon: '⚖️',
        message: 'logged weight', detail: `${weight.weight} kg`,
      ...
      moodData?.forEach(mood => {
        ...
        activities.push({
          id: `mood_${mood.id}`, clientId: mood.client_id, type: 'mood', icon: emoji,
