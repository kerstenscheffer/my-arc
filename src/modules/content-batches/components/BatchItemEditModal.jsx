...
function FieldInput({ field, value, onChange, isFirst }) {
  ...
  if (field.type === 'checklist') {
    ...
    return <div>{label}<div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>{(field.options || []).map(o => {
      ...
      return <button key={o} onClick={() => toggle(o)} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '0.5rem 0.6rem', background: on ? 'rgba(255,215,0,0.1)' : 'rgba(0,0,0,0.3)', border: `1px solid ${on ? 'rgba(255,215,0,0.35)' : 'rgba(255,255,255,0.12)'}`, borderRadius: 6, cursor: 'pointer', textAlign: 'left' }}><span style={{ width: 18, height: 18, borderRadius: 4, flexShrink: 0, background: on ? '#FFD700' : 'transparent', border: on ? 'none' : '2px solid #444', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{on && <Check size={12} strokeWidth={3} color="#000" />}</span><span style={{ fontSize: '0.82rem', color: on ? '#fff' : 'rgba(255,255,255,0.7)' }}>{o}</span></button>
    })}</div></div>
  }
  if (field.type === 'list') {
